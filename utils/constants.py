...
demo_txt_pattern = r"^([^,，]+)(?:[,，])?(?!#genre#)" + r"(" + url_pattern.pattern + r")?"

txt_pattern = r"^([^,，]+)(?:[,，])(?!#genre#)" + r"(" + url_pattern.pattern + r")"

m3u_pattern = r"^#EXTINF:-1.*?(?:，|,)(.*?)\n" + r"(" + url_pattern.pattern + r")"

sub_pattern = re.compile(r"-|_|\((.*?)\)|\（(.*?)\）|\[(.*?)\]|\「(.*?)\」| |｜|频道|普清|标清|高清|HD|hd|超清|超高|超高清|中央|央视|电视台|台|电信|联通|移动")
